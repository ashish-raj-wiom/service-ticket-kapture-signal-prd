...
|---|---|---|---|
| AC-R1-1 | **Given** restore ticket `1789100000000000` with no assignment comment on it, **When** Ramesh Kumar, the Owner on that CSP, accepts the ticket at 09:31, **Then** one comment is added to that Kapture ticket reading `Ticket assigned — Ramesh Kumar (Owner)`. | R1a · R1b | Settled |
| AC-R1-2 | **Given** ticket `1789100000000000` carrying one comment reading `Ticket assigned — Ramesh Kumar (Owner)` from 09:31, **When** Ramesh assigns technician Imran Sheikh at 09:48, **Then** a further comment is added reading `Ticket assigned — Imran Sheikh (Technician)`. | R1a · R1b | Settled |
| AC-R1-3 | **Given** ticket `1789100000000000` with Imran named on the newest comment, **When** Ramesh swaps the job to technician Vikas Yadav at 11:02, **Then** a further comment is added reading `Ticket assigned — Vikas Yadav (Technician)`. | R1a | Settled |
| AC-R1-4 | **Given** ticket `1789100000000000` with Imran named on the newest comment, **When** Ramesh recalls the job off Imran at 11:02 and takes it himself, **Then** a further comment is added reading `Ticket assigned — Ramesh Kumar (Owner)`. | R1a · R1b | Settled |
...
| AC-R2-2 | **Given** the swap to Vikas Yadav at 11:02, **When** its comment is added, **Then** that comment reads `Ticket assigned — Vikas Yadav (Technician)` and names neither Ramesh Kumar nor Imran Sheikh. | R2 MUST NOT | Settled |
...
flowchart TD
    ...
    B -- "No" --> D{"Can the assignee's name and designation be resolved?"}
    ...
    F -- "Yes" --> H["Comment added naming the person and designation — R1a, R1b"]
...
| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-WF-1 | **Given** restore ticket `1789100000000000` reaching the CSP at 09:14, **When** Ramesh accepts at 09:31, assigns Imran at 09:48, swaps to Vikas at 11:02, and resolves at 12:15, **Then** the Kapture ticket carries three assignment comments in that order — `Ticket assigned — Ramesh Kumar (Owner)`, `Ticket assigned — Imran Sheikh (Technician)`, `Ticket assigned — Vikas Yadav (Technician)` — the resolution adds none, and the newest names Vikas. | R1a · R2a · G1 · G2 | Settled |
| AC-WF-2 | **Given** ticket `1789100000000000` reaching the CSP at 09:14, **When** Ramesh accepts at 09:31 and resolves at 09:33, **Then** the ticket carries one assignment comment reading `Ticket assigned — Ramesh Kumar (Owner)`, added before the resolution. | R1a | Settled |
...
| AC-FAIL-2 | **Given** the failed push from AC-FAIL-1, **When** Ramesh swaps to Vikas at 11:02, **Then** a comment reading `Ticket assigned — Vikas Yadav (Technician)` is added — an earlier failure never stops a later action being written. | R1a · G2 | Settled |
...
| Gateway `csp_users` | Read for the assignee's name and designation. | `csp_users` — id, csp_id, role, first name, last name, phone | Role holds exactly four values today — `TECHNICIAN` (3,180), `OWNER` (1,081), `MANAGER` (707), `MANAGER_PLUS` (122) — every one carrying a name and a number, and 19,698 of 19,698 assigned jobs over 30 days resolve to both · AC-R1-1 · AC-R1-2 · AC-R1-9 |
...
|---|---|---|
| Assignee | The person working the ticket — either a user on the CSP who took it themselves (an Owner, Manager or Manager+), or a technician one of them assigned. One at a time per ticket. | CSP execution |
| Assign action | Any action on the CSP side that sets or changes the assignee: a user taking the ticket themselves, assigning a technician, swapping to a different technician, or recalling it off one. | CSP execution |
...
| Designation | What the assignee is, taken from their own user record: Owner, Manager, Manager+ or Technician. A CSP is an organisation, not one person — its Owner, Managers and Managers+ can each take a ticket — so the comment names the individual's designation rather than a blanket word for the firm. | CSP execution |
| Partner queue | The Kapture queue these tickets sit on, and the word the call centre uses for the CSP as a firm. The comment never uses it: it names the person and their designation. | Support/Ops |
...
| Resolve an assignee's name and their own designation — Owner, Manager, Manager+ or Technician — from their identifier, at the moment the comment is written. | R1b · §6 gateway `csp_users` |
